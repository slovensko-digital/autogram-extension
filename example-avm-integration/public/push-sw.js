...
async function saveNotification(item) {
  ...
  await runTransaction(database, "readwrite", (store) => store.put(item));
}

async function getNotifications() {
  const database = await openDatabase();
  const items = await runTransaction(database, "readonly", (store) =>
    store.getAll()
  );
...
async function getNotification(id) {
  const database = await openDatabase();
  return await runTransaction(database, "readonly", (store) => store.get(id));
}
...
async function runTransaction(database, mode, operation) {
  return await new Promise((resolve, reject) => {
